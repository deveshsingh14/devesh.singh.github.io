...
## 2024-08-18 - Character Counter Accessibility Anti-Pattern
**Learning:** Placing `aria-live="polite"` on a dynamic character counter that updates on every keystroke is an accessibility anti-pattern, as it causes screen readers to constantly interrupt the user with each new count while they are trying to type.
**Action:** Use `aria-describedby` on the input to link it to the counter container, rather than using `aria-live` for keystroke-level updates.
